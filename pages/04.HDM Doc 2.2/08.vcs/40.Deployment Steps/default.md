...
This section describes how to deploy, configure, and undeploy PrimaryIO Hybrid Cloud Data Management (HDM) 2.2 for enabling migrations of virtual machines to IBM Cloud for VMware Solutions Dedicated (VCS).
...
Depending on the WAN bandwidth and WAN latency, it may take between 10 to 20  minutes to deploy HDM components in the cloud. Any failure-retries during the deployment process will be logged into vCenter Events. To search for events generated by HDM, **_hdm_** or _com.primaryio_ keywords can be used in vCenter event search.
